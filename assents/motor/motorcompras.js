...
(function () {
  ...
  function sanitizeComentarioObraRow(row) {
    const obraKey = normalizarObraKey(row && (row.obra || row.obra_codigo || row.obra_key));
    return {
      obra: String((row && row.obra) ?? "").trim(),
      obra_key: obraKey,
      obra_codigo: formatObraCodigo(obraKey),
      usuario: String((row && (row.usuario || row.autor || row.criado_por)) ?? "").trim(),
      comentario: String((row && (row.comentario || row.mensagem || row.texto)) ?? "").trim(),
      criado_em: String((row && (row.criado_em || row.data || row.atualizado_em)) ?? "").trim()
    };
  }
  ...
  async function fetchComentariosObra(obraOuKey) {
    const obraKey = normalizarObraKey(obraOuKey);
    if (!obraKey) return [];

    if (cacheComentariosPorObra && cacheComentariosPorObra[obraKey]) {
      return cacheComentariosPorObra[obraKey].map(row => Object.assign({}, row));
    }

    if (cacheComentarios) {
      cacheComentariosPorObra = agruparComentariosPorObra(cacheComentarios);
      return (cacheComentariosPorObra[obraKey] || []).map(row => Object.assign({}, row));
    }

    if (cacheComentariosPromise) {
      await cacheComentariosPromise;
      return (cacheComentariosPorObra && cacheComentariosPorObra[obraKey] ? cacheComentariosPorObra[obraKey] : []).map(row => Object.assign({}, row));
    }

    cacheComentariosPromise = fetchApiSheet("comentarios_obra")
      .then(data => {
        cacheComentarios = data.map(sanitizeComentarioObraRow).filter(row => row.obra_key && row.comentario);
        cacheComentariosPorObra = agruparComentariosPorObra(cacheComentarios);
        return cacheComentarios.slice();
      })
      .catch(() => {
        cacheComentarios = [];
        cacheComentariosPorObra = {};
        return [];
      })
      .finally(() => {
        cacheComentariosPromise = null;
      });
  ...
  function agruparVinculosPorObraItem(vinculos) {
    ...
    vinculos.forEach(row => {
      ...
      const ped = String(row.pednum || '').trim();
      const jaExiste = ped && result[key].some(item => String(item.pednum || '').trim() === ped);
      if (!jaExiste) result[key].push(row);
    });

    Object.keys(result).forEach(key => {
      result[key].sort((a, b) => String(a.pednum || '').localeCompare(String(b.pednum || ''), 'pt-BR', { numeric: true }));
    });
  ...
  function agruparComentariosPorObra(comentarios) {
    const result = {};
    (Array.isArray(comentarios) ? comentarios : []).forEach(row => {
      if (!row.obra_key || !row.comentario) return;
      if (!result[row.obra_key]) result[row.obra_key] = [];
      result[row.obra_key].push(row);
    });

    Object.keys(result).forEach(key => {
      result[key].sort((a, b) => String(a.criado_em || "").localeCompare(String(b.criado_em || "")));
    });
  ...
  function getResumoObraSync(obraOuKey) {
    const key = normalizarObraKey(obraOuKey) || String(obraOuKey || "").replace(/\D/g, "");
    const base = key && cachePorObra && cachePorObra[key] ? cachePorObra[key] : {
      obra_key: key,
      obra_codigo: formatObraCodigo(key),
  ...
  function buildSalvarCpmvUrl(obra, cpmvPlanejado, observacao, atualizadoPor, senhaCpmv) {
    ...
    if (senhaCpmv) params.set("senha_cpmv", String(senhaCpmv || "").trim());
  ...
  async function salvarCpmvPlanejado(obra, cpmvPlanejado, observacao = "", atualizadoPor = "interface", senhaCpmv = "") {
    ...
    const url = buildSalvarCpmvUrl(formatObraCodigo(obraKey), valor, observacao, atualizadoPor, senhaCpmv);
    const payload = await fetchJson(url).catch(() => fetchJsonp(url));
  ...
  async function getVinculosOcItem(obraOuKey, item) {
    ...
    const vinculos = cacheVinculosPorObraItem && Array.isArray(cacheVinculosPorObraItem[key])
      ? cacheVinculosPorObraItem[key]
      : [];

    return vinculos.map(v => Object.assign({}, v));
  }

  function getVinculosOcItemSync(obraOuKey, item) {
    const obraKey = normalizarObraKey(obraOuKey) || String(obraOuKey || "").replace(/\D/g, "");
    const itemKey = normalizarItemKey(item);
    const key = `${obraKey}::${itemKey}`;
    const vinculos = cacheVinculosPorObraItem && Array.isArray(cacheVinculosPorObraItem[key])
  ...
  async function getVinculoOcItem(obraOuKey, item) {
    const vinculos = await getVinculosOcItem(obraOuKey, item);
    return vinculos.length ? Object.assign({}, vinculos[0]) : null;
  }

  function getVinculoOcItemSync(obraOuKey, item) {
    const vinculos = getVinculosOcItemSync(obraOuKey, item);
    return vinculos.length ? Object.assign({}, vinculos[0]) : null;
  }
  ...
  async function salvarVinculoOcItem(obra, item, compra, criadoPor = "interface") {
    ...
    const idx = cacheVinculos.findIndex(v =>
      v.obra_key === obraKey &&
      v.item_key === itemKey &&
      String(v.pednum || '').trim() === String(row.pednum || '').trim()
    );
  ...
  function buildSalvarComentarioObraUrl(payload) {
    const params = new URLSearchParams();
    params.set("action", "salvar_comentario_obra");
    params.set("obra", String(payload.obra || "").trim());
    params.set("usuario", String(payload.usuario || "").trim());
    params.set("comentario", String(payload.comentario || "").trim());
    if (CPMV_WRITE_TOKEN) params.set("token", CPMV_WRITE_TOKEN);
  ...
  async function salvarComentarioObra(obra, usuario, comentario) {
    const obraKey = normalizarObraKey(obra);
    const nomeUsuario = String(usuario || "").trim();
    const texto = String(comentario || "").trim();

    if (!obraKey) throw new Error("Obra inválida para salvar comentário.");
    if (!nomeUsuario) throw new Error("Informe o usuário do comentário.");
    if (!texto) throw new Error("Informe o comentário.");
    ...
    const url = buildSalvarComentarioObraUrl(payloadLocal);
    const response = await fetchJson(url).catch(() => fetchJsonp(url));

    if (!response || response.ok === false) {
      throw new Error((response && response.error) || "Não foi possível salvar o comentário.");
    }

    const row = sanitizeComentarioObraRow(Object.assign({}, payloadLocal, response.data || {}));
    row.obra_key = obraKey;
    row.obra_codigo = formatObraCodigo(obraKey);

    if (!cacheComentarios) cacheComentarios = [];
    cacheComentarios.push(row);
    cacheComentariosPorObra = agruparComentariosPorObra(cacheComentarios);
  ...
  window.motorCompras = {
    ...
    fetchComentariosObra,
    getResumoObra,
    getResumoObraSync,
    getVinculosOcItem,
    getVinculosOcItemSync,
    getVinculoOcItem,
