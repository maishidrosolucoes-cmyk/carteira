const ITENS = ["BBA/ELET.", "MT", "FLUT.", "M FV.", "AD. FLEX", "AD. RIG.", "FIXADORES", "SIST. ELÉT.", "PEÇAS REP.", "SERV.", "MONT.", "FATUR."];
  ...
  function expandirLinhasEntreguePorPedido(dados) {
    return (Array.isArray(dados) ? dados : []).flatMap(item => {
      ...
      return pedidos
        .filter(pedido => pedido && isStatusEntregue(pedido.status_operacional))
        .map((pedido, idx) => {
          const rowClone = row.slice();
          const metaObraKey = String(pedido.meta_obra_key || detalhes.meta_obra_key || '').trim();
          const numeroPedido = String(pedido.numero_pedido || '').trim();
          const valorPedido = parseMoneyFlexible(pedido.valor);
          const dataFaturamento = String(pedido.data_faturamento || '').trim();
          const docsOriginais = Array.isArray(pedido.meta_concluidas_nf) ? pedido.meta_concluidas_nf : [];
          const docs = docsOriginais.length
            ? docsOriginais
            : (dataFaturamento && valorPedido > 0 ? [{
                nf: String(pedido.nf || '').trim(),
                valor: valorPedido,
                item: String(pedido.item || '').trim(),
                categoria: String(pedido.categoria || '').trim(),
